import type { VideoAnnotation, ResolvedCitation, Source, ClaimType, Confidence } from "@citecast/shared";
...
function generateBibTeX(citations: ResolvedCitation[]): string {
...
function generateMarkdown(citations: ResolvedCitation[]): string {
...
export function createSidebar(
  ...
  let allCitations: ResolvedCitation[] = [];
  let isOpen = false;

  // Toggle button
  const toggleBtn = document.createElement("button");
  toggleBtn.className = "citecast-sidebar-toggle";
  ...
  sidebar.className = "citecast-sidebar";
  ...
  function renderList() {
    ...
    title.textContent = `Citations (${filtered.length})`;
    ...
    filtered.forEach((citation) => {
      ...
      timestamp.className = "citecast-citation-timestamp";
      timestamp.textContent = formatTime(citation.start_time);
      timestamp.title = `Seek to ${formatTime(citation.start_time)}`;
      timestamp.addEventListener("click", (e) => {
        e.stopPropagation();
        videoElement.currentTime = citation.start_time;
      });
  ...
  function openSidebar() {
    isOpen = true;
    sidebar.classList.add("citecast-sidebar--open");
    toggleBtn.classList.add("citecast-sidebar-toggle--active");
  }

  function closeSidebar() {
    isOpen = false;
    sidebar.classList.remove("citecast-sidebar--open");
